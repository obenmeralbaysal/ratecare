                    ...
                    <div class="body table-responsive">
                        <table class="table m-b-0">
                            ...
                            @foreach($users as $user)
                                <tr>
                                    ...
                                    <td class="td-namesurname">
                                        <a href="{{ route("admin.users.switchUser", $user->id) }}">
                                            {{ $user->namesurname }}
                                        </a>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-center">
                                        @if(isReseller($user))
                                            RESELLER ACCOUNT
                                        @else
                                            {{ object_get($user, 'reseller.namesurname', '-- NA --') }}
                                        @endif
                                    </td>
                                    <td class="text-nowrap">{{ formatDate($user->created_at) }}</td>
                                    <td class="text-nowrap text-right">
                                        <a href="{{ route("admin.users.switchUser",
                            ...
                            @foreach($invited as $invite)
                                <tr>
                                    <th scope="row">-</th>
                                    <td>{{ $invite->namesurname }}</td>
                                    <td>{{ $invite->email }}</td>
                                    <td>
                                        <button class="btn btn-warning btn-sm">Invited</button>
                                        <a href="{{ route("admin.users.deleteInvitation", $invite->id) }}" onclick="return confirm('Are you sure?')">
                                            <span class="btn btn-danger btn-sm btn-red"><i class="zmdi zmdi-delete"></i></span>
                                        </a>
                                    </td>
                                </tr>
                            @endforeach
